...
import { useEditUser, EditUserFormValues } from "../hooks/useEditUser"
...
export function EditUserModal({ userToEdit, isOpen, onClose }: EditUserModalProps) {
  ...
  const { form, onSubmit, isSubmitting } = useEditUser({ userToEdit, isOpen, onClose })
      ...
      <DialogContent className="bg-white/95 backdrop-blur-2xl border-white/60 shadow-2xl rounded-[2rem] sm:max-w-[550px] p-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
        <DialogHeader className="mb-4 border-b border-gray-100 pb-4">
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">
            Modifica la información o resetea la contraseña de {userToEdit?.first_name}.
          </p>
        </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="password" placeholder="En blanco para no cambiar" className="bg-gray-50/50 h-11 rounded-xl focus:bg-white" disabled={isSubmitting} {...field} />
                    </FormControl>
                    <FormMessage className="text-xs" />
                  </FormItem>
                )}
